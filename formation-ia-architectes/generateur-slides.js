...
function promptBox(s, x, y, w, h, title, txt, fs, bg) {
  card(s, x, y, w, h, bg || "1B2126");
...
/* ============ 1. TITRE ============ */
{
  ...
  s.addText("FORMATION · ARCHITECTES & MAÎTRISE D'ŒUVRE", { x: M, y: 1.02, w: 8.2, h: 0.3,
    isTextBox: true, margin: 0, fontFace: BODY, fontSize: 11.5, bold: true, color: ACCENT, charSpacing: 1.8 });
  s.addText("L'IA au service\ndu projet architectural", { x: M, y: 1.42, w: 8.4, h: 2.1,
    isTextBox: true, margin: 0, fontFace: HEAD, fontSize: 46, bold: true, color: "FFFFFF", lineSpacingMultiple: 1.05 });
  s.addText("Conducteur de session — support animateur", { x: M, y: 3.62, w: 8.2, h: 0.4,
    isTextBox: true, margin: 0, fontFace: BODY, fontSize: 16, italic: true, color: DKMUT });
  s.addText("INTERVENANT", { x: M, y: 4.3, w: 8.2, h: 0.26, isTextBox: true, margin: 0,
    fontFace: BODY, fontSize: 9.5, bold: true, color: ACCENT, charSpacing: 1.6 });
  s.addText("Anthony — ANCA Agency", { x: M, y: 4.56, w: 8.2, h: 0.38, isTextBox: true, margin: 0,
    fontFace: HEAD, fontSize: 19, bold: true, color: "FFFFFF" });
  s.addText("anthony@anca-agency.com   ·   [téléphone]", { x: M, y: 4.96, w: 8.2, h: 0.3,
    isTextBox: true, margin: 0, fontFace: BODY, fontSize: 11.5, color: DKMUT });
  [["3 h 00", "180 minutes, sans temps mort"], ["2", "participants maximum"],
   ["3", "outils : ChatGPT · Seedance 2.0 · Claude"]].forEach((f, i) => {
    const x = M + i * 2.75;
    s.addText(f[0], { x, y: 5.58, w: 2.6, h: 0.58, isTextBox: true, margin: 0,
      fontFace: HEAD, fontSize: 27, bold: true, color: "FFFFFF" });
    s.addText(f[1], { x, y: 6.16, w: 2.6, h: 0.5, isTextBox: true, margin: 0,
      fontFace: BODY, fontSize: 10.5, color: DKMUT });
  });
  footer(s, true);
...
/* ============ 2. LE FIL DE LA SESSION ============ */
{
  ...
  const rows = [
    ["00:00", "15 min", "CADRAGE", "Ce qu'on s'autorise, et ce qu'on s'interdit"],
    ["00:15", "30 min", "LE PROMPT, VRAIMENT", "Un squelette réutilisable, écrit avec eux"],
    ["00:45", "45 min", "L'IMAGE PHOTORÉALISTE", "Une image de leur projet, tenue de bout en bout"],
    ["01:30", "10 min", "PAUSE", ""],
    ["01:40", "35 min", "LA VIDÉO COURTE", "Un clip de six secondes, prêt à publier"],
    ["02:15", "35 min", "CRÉER SON AGENT", "Un assistant qui tourne avant la fin de la séance"],
    ["02:50", "10 min", "PASSAGE À L'ÉCHELLE", "Une décision d'agence, et un plan à trente jours"]
  ];
  let y = 1.52;
  rows.forEach(r => {
...
/* ============ 4. § CADRAGE ============ */
{
  const s = divider("1", "Cadrage", "00:00 → 00:15  ·  15 MINUTES",
    "Ce qui tient ses promesses, et ce qui n'est qu'une démo.");
  const items = [["3 min", "Tour de table : leur usage réel, sans complaisance"],
    ["5 min", "Trois outils — et ce qu'on ne fera pas avec les autres"],
    ["5 min", "Droits d'auteur, RGPD, concours : les quatre interdits"],
    ["2 min", "La phrase qu'ils entendront trois fois dans la journée"]];
  const cw = (CW - 0.75) / 4;
  items.forEach((it, i) => {
    const x = M + i * (cw + 0.25);
...
/* ============ 7. § LE PROMPT ============ */
{
  ...
  const items = [["9 min", "La structure en six blocs, écrite avec eux sur un vrai projet"],
    ["6 min", "Le vocabulaire utile — et celui qui ne sert à rien"],
    ["15 min", "Exercice comparatif, chacun sur son propre projet"]];
  const cw = (CW - 0.5) / 3;
  items.forEach((it, i) => {
    const x = M + i * (cw + 0.25);
    card(s, x, 3.62, cw, 1.58, DKCARD);
...
/* ============ 9. VOCABULAIRE (utile + inutile) ============ */
{
  ...
  [["« professional photo »", "« verticales redressées, objectif 24 mm »"],
   ...
   ["« wood facade »", "« brique de parement rouge, joints creux »"],
...
/* ============ 10. MAUVAIS PROMPT / BON PROMPT ============ */
{
  const s = light("Le contre-exemple", "Le même projet, deux prompts", "00:27");
  const half = (CW - 0.4) / 2;
  const x2 = M + half + 0.4;

  s.addText("CE QU'ON ÉCRIT SPONTANÉMENT", { x: M, y: 1.46, w: half, h: 0.26, isTextBox: true,
    margin: 0, fontFace: BODY, fontSize: 10, bold: true, color: ACCENT, charSpacing: 1.2 });
  s.addText("CE QUI PRODUIT VOTRE PROJET", { x: x2, y: 1.46, w: half, h: 0.26, isTextBox: true,
    margin: 0, fontFace: BODY, fontSize: 10, bold: true, color: SLATE, charSpacing: 1.2 });

  promptBox(s, M, 1.74, half, 1.42, null,
    "Une belle maison contemporaine en bois,\ntrès moderne, avec une jolie lumière.\nRendu photoréaliste 8K, ultra realistic,\nmasterpiece, award winning,\nunreal engine 5.", 10, "2E1C16");
  promptBox(s, x2, 1.74, half, 1.42, null,
    "Voici une capture de ma maquette 3D.\nConserve la géométrie et le point de vue.\nBardage mélèze vertical, lames de 12 cm.\nSoubassement béton brut lisse.\nFin d'après-midi, ombres longues vers l'est.\nObjectif 24 mm, verticales redressées.", 10);

  card(s, M, 3.26, half, 1.34, "F7E9E4");
  label(s, M + 0.3, 3.44, half - 0.6, "Les quatre fautes", ACCENT);
  bullets(s, M + 0.3, 3.74, half - 0.6, 0.82, [
    "Aucune géométrie : ce ne sera jamais votre maison.",
    "Aucun cadrage : ni focale, ni hauteur, ni orientation.",
    "« en bois » : quelle essence, quel sens de pose ?",
    "Quatre superlatifs à la place de quatre contraintes."], 10.5, RUST, 2);

  card(s, x2, 3.26, half, 1.34, CARD);
  label(s, x2 + 0.3, 3.44, half - 0.6, "Ce qui a changé", SLATE);
  bullets(s, x2 + 0.3, 3.74, half - 0.6, 0.82, [
    "La géométrie vient de vous, pas du modèle.",
    "Le cadrage est écrit : 24 mm, verticales redressées.",
    "Chaque matériau est nommé, avec sa mise en œuvre.",
    "Le même prompt redonne la même image demain."], 10.5, SLATE, 2);

  label(s, M, 4.72, CW, "Trois autres prompts à ne pas écrire", ACCENT);
  const cw3 = (CW - 0.5) / 3;
  [["« Fais-moi un rendu de mon plan »", "Un plan n'est pas une vue. Joignez une capture 3D en perspective."],
   ["« Change le bardage, mets une ambiance nuit et enlève la voiture »", "Trois demandes en une : il en applique une et demie."],
   ["« Rends ça plus beau », « Améliore l'image »", "Aucune consigne exploitable : il redessine tout et vous perdez vos 80 %."]
  ].forEach((p, i) => {
    const x = M + i * (cw3 + 0.25);
    card(s, x, 5.0, cw3, 1.02, CARD2);
    s.addText(p[0], { x: x + 0.24, y: 5.14, w: cw3 - 0.48, h: 0.42, isTextBox: true, margin: 0,
      valign: "top", fontFace: BODY, fontSize: 10.5, bold: true, italic: true, color: INK });
    s.addText(p[1], { x: x + 0.24, y: 5.56, w: cw3 - 0.48, h: 0.4, isTextBox: true, margin: 0,
      valign: "top", fontFace: BODY, fontSize: 10, color: SLATE });
  });

  tip(s, "Faites-leur écrire le mauvais prompt d'abord, et générez-le vraiment. Une image ratée qu'ils ont produite eux-mêmes vaut dix explications sur ce qu'il ne faut pas faire.");
  notes("3 minutes. À montrer juste avant l'exercice comparatif — c'est la grille de lecture qu'ils vont appliquer dans les quinze minutes suivantes.\n\nLire le mauvais prompt à voix haute : il fait rire, parce que c'est exactement ce qu'ils ont écrit la première fois qu'ils ont essayé. C'est le moment où ils se reconnaissent.\n\nLe point à marteler : le mauvais prompt n'est pas mauvais parce qu'il est trop court. Il est mauvais parce qu'il ne contient AUCUNE contrainte vérifiable. Les deux prompts font à peu près la même longueur — c'est le contenu qui change, pas le volume.\n\nLes trois prompts du bas sont ceux que vous verrez pendant les ateliers. Les avoir annoncés vous permet de dire « c'est le cas 2 » au lieu de réexpliquer.\n\nLe troisième, « rends ça plus beau », est le plus coûteux : il détruit les 80 % déjà acquis. Faire le lien avec la grille d'itération qui arrive à 01:12.");
}

/* ============ 11. ATELIER EXERCICE COMPARATIF ============ */
{
...
/* ============ 12. ATELIER CAPTURE 3D ============ */
{
  ...
  promptBox(s, M + half + 0.4, 1.52, half, 4.5, "Le prompt type — à copier dans la bibliothèque",
    "Voici une capture de ma maquette 3D.\n\nConserve exactement la géométrie, les\nproportions, le cadrage et le point de vue.\nN'ajoute ni ne supprime aucun volume,\naucune ouverture, aucun niveau.\n\nTransforme-la en photographie d'architecture :\n\n— Brique de parement rouge, joints creux\n— Attique en zinc à joint debout gris quartz\n— Menuiseries bois, garde-corps métal noir\n— Milieu de matinée, ciel dégagé, ombres\n  franches portées vers le nord-ouest\n— Trois passants et un vélo, à l'échelle\n— Objectif 35 mm, verticales redressées\n\nLe sol, le ciel et la végétation : libres.\nLa façade, non.");
...
/* ============ 14. GRILLE D'ITÉRATION ============ */
{
  ...
  const rows = [
    ["Une matière", "« Garde tout identique. Change uniquement le parement en pierre agrafée beige clair. »"],
...
/* ============ 17. § VIDÉO (avec la pause) ============ */
{
  ...
  const items = [["5 min", "Seedance 2.0 : capacités, accès, coût réel"],
    ["8 min", "Les six blocs du prompt vidéo, et les six pièges"],
    ["4 min", "Quatre mouvements de caméra, un seul par plan"],
    ["20 min", "Atelier : animer l'image validée au module précédent"]];
  const cw = (CW - 0.75) / 4;
  items.forEach((it, i) => {
    const x = M + i * (cw + 0.25);
    card(s, x, 3.62, cw, 1.58, DKCARD);
...
/* ============ 22. § AGENT ============ */
{
  ...
  const items = [["5 min", "De la conversation jetable à l'assistant qui vous connaît"],
    ...
    ["5 min", "Les cinq blocs d'instructions d'un agent d'agence"],
    ["20 min", "Construction en direct, sur leurs propres documents"]];
  const cw = (CW - 0.75) / 4;
  items.forEach((it, i) => {
    const x = M + i * (cw + 0.25);
    card(s, x, 3.62, cw, 1.58, DKCARD);
  ...
  tip(s, "Le piège de ce module : passer trente-cinq minutes à expliquer ce qu'est un agent. La construction démarre à 02:30, montre en main, même si l'explication n'est pas finie.", true);
...
/* ============ 24. ANATOMIE DE L'AGENT ============ */
{
  ...
  const blocks = [
    ...
    ["Références", "Ce sur quoi il s'appuie.", "Tu t'appuies exclusivement sur les documents déposés dans le projet. Tu ne t'inspires jamais d'exemples génériques trouvés ailleurs."],
...
/* ============ 26. § PASSAGE À L'ÉCHELLE ============ */
{
  ...
  const items = [["3 min", "Quels outils garder, à quel budget"],
    ...
    ["3 min", "Les quatre livrables, vérifiés un par un"]];
  const cw = (CW - 0.5) / 3;
  items.forEach((it, i) => {
    const x = M + i * (cw + 0.25);
    card(s, x, 3.62, cw, 1.58, DKCARD);
...
/* ============ 28. CE QUE VOUS EMPORTEZ ============ */
{
  ...
  card(s, M, 5.08, CW, 1.24, ACCENT);
  s.addText("Et une phrase : une image générée est une image d'intention.", {
    x: M + 0.35, y: 5.24, w: CW - 0.7, h: 0.48, isTextBox: true, margin: 0,
  ...
  s.addText("Jamais un rendu conforme au modèle. Jamais une étude d'ensoleillement. C'est ce qui vous autorise à vous en servir devant un client.", {
    x: M + 0.35, y: 5.76, w: CW - 0.7, h: 0.4, isTextBox: true, margin: 0,
    fontFace: BODY, fontSize: 13, color: "F6DCD1" });
  s.addText("Anthony — ANCA Agency   ·   anthony@anca-agency.com   ·   [téléphone]", {
    x: M, y: 6.44, w: CW, h: 0.32, isTextBox: true, margin: 0,
    fontFace: BODY, fontSize: 12, bold: true, color: DKMUT });
  footer(s, true);
...
/* ============ 29. ANTISÈCHE ANIMATEUR ============ */
{
  ...
  bullets(s, M + 0.3, 4.8, CW - 0.6, 1.1, [
    ...
    11.5, "C8D0D4", 6);
  tip(s, "Arrivez une heure avant. La moitié des incidents de session sont des problèmes de connexion ou de comptes, et ils se règlent tous avant que les participants entrent dans la salle.");
